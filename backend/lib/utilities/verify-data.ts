...
async function verify() {
  ...
  const [
    ...
    supabase.from('mn_tasks').select('*', { count: 'exact', head: true }),
    supabase.from('mn_errors').select('*', { count: 'exact', head: true }),
  ]);
  ...
  if (mentorsCount === tasksCount) {
    console.log('✅ Tables have matching counts!\n');
  } else {

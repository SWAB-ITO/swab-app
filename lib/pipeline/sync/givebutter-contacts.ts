...
async function syncContacts() {
  ...
  const [{ data: signups }, { data: setup }, { data: members }] = await Promise.all([
    supabase.from('jotform_signups_raw').select('uga_email, personal_email, phone'),
    supabase.from('jotform_setup_raw').select('email, phone'),
    supabase.from('givebutter_members_raw').select('email, phone')
  ]);
  ...
  for await (const row of parser) {
    ...
    if (batch.length >= BATCH_SIZE) {
      await processBatch(
        batch.splice(0, BATCH_SIZE).map(c => ({

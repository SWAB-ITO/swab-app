...
function validateCSV(filepath: string): ValidationResult {
  ...
  records.forEach((record: any, index: number) => {
    ...
    // Validation 5: Custom field length (Givebutter max: 255 chars)
    const textInstructions = record['Text Instructions'];
    if (textInstructions && textInstructions.length > 255) {
      issues.push({
        row: rowNum,
        field: 'Text Instructions',
        issue: `Exceeds 255 character limit (${textInstructions.length} chars)`,
        severity: 'error',
      });
    }
    ...
    if (status && status.length > 255) {
      issues.push({
        row: rowNum,
        field: '$ Status',
        issue: `Exceeds 255 character limit (${status.length} chars)`,
        severity: 'error',
      });
    }
    ...
    // Validation 8: Status value
    const validStatuses = ['needs_setup', 'needs_page_creation', 'needs_fundraising', 'fully_complete'];
    if (status && !validStatuses.includes(status)) {
      issues.push({
        row: rowNum,
        field: '$ Status',
        issue: `Unknown status value: ${status} (expected one of: ${validStatuses.join(', ')})`,
        severity: 'warning',
      });
    }
...
if (filename) {
  filepath = resolve(process.cwd(), 'data', filename);
} else {
